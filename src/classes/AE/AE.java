...
public class AE {
    ...
    // wszystko co związane z tworzeniem i zarządzaniem chromosomami (Czyli algorytm
    // ewolucyjny)
    ...
    public static void WypiszInfoOPopulacji() {// zmienioned
        ...
        for (int i = 0; i < Chromosom.populacja.size(); i++) {
            System.out.println("Osobnik nr " + (i + 1) + "    Przystosowanie: "
                    + String.format("%.5f", Chromosom.populacja.get(i).przystosowanieChromosomu));
        }
    ...
    public static void SelekcjaRankingowa(int KoncowawielkoscPopulacji) {
        ...
        for (int i = 0; i < Chromosom.populacja.size(); i++) {
            for (int j = 1; j < Chromosom.populacja.size(); j++) {
                if (Chromosom.populacja.get(j - 1).przystosowanieChromosomu > Chromosom.populacja
                        .get(j).przystosowanieChromosomu) {
    ...
    public static void KrzyzujChromosomy(Chromosom chromosom1, Chromosom chromosom2) {
        ...
        int IndexKoncowy = nieDoKońcaLosowaLiczba + IndexPoczatekowy;// odległośc między indeksami jest mniejsza ale
                                                                     // zawsze równa conajmniej 5;
    ...
    // mutacja (inwersja) // inwersja to zamiana 2 sąsiadujących mutacja 2 w
    // losowych miejscach
    ...
    // populacja to największa ilość trans między miastami (na start 20, a później
    // do zmiany)
    ...
    // funkcja przystosowania danego chromosomu to suma drogi między wszystkimi
    // miastami w kolejności wskazanej przez geny chromosomu
    // optymalizacja polega na znalezieniu jak najkrótszej drogi pozwalającej na
    // przebycie wszystkich wskazanych miast
